...
def Initialize(guiRoot):
    ...
    # Widget Setup
    buttonLabels   = ["Endurance\nView", "Lap\nView", "Testing\nView", "Settings"]
    buttonCommands = [lambda: gui.SetView(gui_endurance),0,0,0]
    ...
    speedStat    = lib_tkinter.DashLabelStat  (display, column=1, row=1, font=config.driveCenterPanelStatFont)
    speedLabel   = lib_tkinter.DashLabelCanvas(display, speedLabelWidth, speedLabelHeight, 40, speedLabelHeight - 16, column=2, row=1, text="MPH", font=config.driveCenterPanelLabelFont)
    ...
    statPanel.rowconfigure   (5, minsize=statPanelDividerHeight)

    # Widgets
    statPanelDividerTop     = lib_tkinter.DashDivider  (statPanel, statPanelInteriorWidth, "Horizontal", column=0, row=0, columnspan=2)
    chargeStat              = lib_tkinter.DashLabelStat(statPanel, column=1, row=1, sticky="E", font=config.driveSidePanelStatFont)
    temp1Stat               = lib_tkinter.DashLabelStat(statPanel, column=1, row=2, sticky="E", font=config.driveSidePanelStatFont, paletteDomain=config.tempDomain, highlightPalette=config.tempHighlight)
    temp2Stat               = lib_tkinter.DashLabelStat(statPanel, column=1, row=3, sticky="E", font=config.driveSidePanelStatFont, paletteDomain=config.tempDomain, highlightPalette=config.tempHighlight)
    temp3Stat               = lib_tkinter.DashLabelStat(statPanel, column=1, row=4, sticky="E", font=config.driveSidePanelStatFont, paletteDomain=config.tempDomain, highlightPalette=config.tempHighlight)
    chargeLabel             = lib_tkinter.DashLabel    (statPanel, column=0, row=1, sticky="W", font=config.driveSidePanelLabelFont, text="SoC:")
    temp1Label              = lib_tkinter.DashLabel    (statPanel, column=0, row=2, sticky="W", font=config.driveSidePanelLabelFont, text="Temp 1:")
    temp2Label              = lib_tkinter.DashLabel    (statPanel, column=0, row=3, sticky="W", font=config.driveSidePanelLabelFont, text="Temp 2:")
    temp3Label              = lib_tkinter.DashLabel    (statPanel, column=0, row=4, sticky="W", font=config.driveSidePanelLabelFont, text="Temp 3:")
    statPanelDividerBottom  = lib_tkinter.DashDivider  (statPanel, statPanelInteriorWidth, "Horizontal", column=0, row=5, columnspan=2)
...
def Update():
    ...
    rpmBar.Set    (car_data.motorRpm)
    torqueBar.Set (car_data.torquePercentageMax)
    regenBar.Set  (car_data.torquePercentageRegen)
    speedStat.Set (car_data.motorSpeedMph)
    chargeStat.Set(car_data.stateOfCharge)
    temp1Stat.Set (car_data.inverterTempMax)
    temp2Stat.Set (car_data.motorTemperature)
    temp3Stat.Set (car_data.packTemperatureMax)
